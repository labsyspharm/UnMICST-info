...
class DNA_GaussianAug(data.Dataset):
    """DNA_GaussianAug
    ...
    def __getitem__(self, index):
        ...
        ### DNA RGB with Gaussian blur inputs ###
        sigma_list = [0, 0.5, 0.75, 1]
        index = math.floor(4*np.random.rand()) #0,1,2,3
        torch_img = []
        for i in range(3):
            ch_img = Image.fromarray(img[0, ...])
            ch_img = gaussian(np.asanyarray(ch_img),sigma=sigma_list[index]) #takes in numpy
