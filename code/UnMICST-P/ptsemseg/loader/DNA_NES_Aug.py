...
class DNA_NES_Aug(data.Dataset):
    """DNA_NES_Aug
    ...
    def __getitem__(self, index):
        ...
        ### DNA and NES with Aug ###
        torch_img = []
        channel = random.choice([0,1,2,3,4,5]) # randomly choose a DAPI channel to train from
        for i in range(3):
            if (i == 2):
                ...
                # # uniform noise of size (64,64) if img is (12,64,64)
                # ch_img = np.random.uniform(size=(np.shape(img)[1],np.shape(img)[2]))
