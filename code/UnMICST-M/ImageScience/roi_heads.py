...
def _onnx_expand_boxes(boxes, scale):
    ...
    boxes_exp = torch.stack(
        (boxes_exp0, boxes_exp1, boxes_exp2, boxes_exp3), 1)
    return boxes_exp
...
def _onnx_paste_mask_in_image(mask, box, im_h, im_w):
    ...
    mask = torch.nn.functional.interpolate(mask, size=(
        int(h), int(w)), mode='bilinear', align_corners=False)
    mask = mask[0][0]
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels, bg_maps):
        # type: (List[Tensor], List[Tensor], List[Tensor])
        matched_idxs = []
        labels = []
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image, bg_map in zip(proposals, gt_boxes, gt_labels, bg_maps):
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                # Label ignore proposals (if bbox proposal's center is "ignore label")
                proposal_centers = torch.stack(
                    [proposals_in_image[:, 0::2].sum(1)/2,
                    proposals_in_image[:, 1::2].sum(1)/2], 1)
                # need to adjust
                proposal_centers = proposal_centers * 64. / 800. 
                proposal_centers = proposal_centers.type(torch.int16)
                before_sum = ignore_inds.sum()
                for p_id, p_center in enumerate(proposal_centers):
                    if bg_map[0, p_center[1], p_center[0]] == 1:
                        ignore_inds[p_id] = True
    ...
    def select_training_samples(self, proposals, targets):
        ...
        # added to ignore labels
        bg_maps = [t["bg_maps"] for t in targets]
        ...
        matched_idxs, labels = self.assign_targets_to_proposals(
            proposals, gt_boxes, gt_labels, bg_maps)
